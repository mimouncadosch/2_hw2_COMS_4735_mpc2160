import numpy as np
import cv2
from texture import texture_similarity
...
def texture_similarity_matrix(b, b_lb):
    ...
    for i in xrange(0, 40):
        for j in xrange(0, 40):
            M[j, i] = texture_similarity(i+1, j+1, b, b_lb)

    return M


def texture_montage(M):
    ...
    for i in xrange(0, 40):
        ...
        # cv2.imshow('m', montage)
        # cv2.waitKey(0)

    # cv2.imwrite('texture_similarities.png', montage)
    # cv2.imshow('texture_similarities', montage)
    # cv2.waitKey(0)
